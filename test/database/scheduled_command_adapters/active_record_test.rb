...
require_relative '../../../lib/kommando/scheduled_command_adapters/active_record'
require_relative '../../../lib/kommando'

class KommandoScheduledCommandAdaptersActiveRecordTest < DatabaseTest
  include Kommando

  def setup
    ActiveRecord::Base.establish_connection(adapter: 'postgresql', database: 'kommando_test')
    ActiveRecord::Base.connection.execute('DELETE FROM kommando_scheduled_commands;')
  end
  ...
  def test_schedule_writes_to_database
    ...
    ScheduledCommandAdapters::ActiveRecord.schedule!(command, parameters, handle_at)

    record = ScheduledCommandAdapters::ActiveRecord.first
  ...
  def test_fetch_yields_callback_with_next_scheduled_command
    ...
    ScheduledCommandAdapters::ActiveRecord.schedule!(command_a, parameters_a, handle_at_a)
    ScheduledCommandAdapters::ActiveRecord.schedule!(command_b, parameters_b, handle_at_b)

    fetched_command = nil
    fetched_parameters = nil

    ScheduledCommandAdapters::ActiveRecord.fetch! do |command, parameters|
  ...
  def test_fetch_removes_successfully_handled_commands
    ...
    assert_equal 0, ScheduledCommandAdapters::ActiveRecord.count
  end
  ...
  def test_fetch_reschedules_commands_that_failed
    ...
    assert_equal 1, ScheduledCommandAdapters::ActiveRecord.count
    assert_equal command, record.name
    assert_equal parameters, record.parameters
  ...
  def test_metrics
    ...
    ScheduledCommandAdapters::ActiveRecord.schedule!(command, parameters_a, handle_at_a)
    ScheduledCommandAdapters::ActiveRecord.schedule!(command, parameters_b, handle_at_b)

    ScheduledCommandAdapters::ActiveRecord.fetch! do |command, parameters|
      CommandResult.failure({ error: :error, command: command, parameters: parameters, details: :details })
    end

    result = ScheduledCommandAdapters::ActiveRecord.metrics
